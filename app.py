...
def free_lesson():
    ...
    if request.method == "POST":
        ...
        phone = (request.form.get("phone") or "").strip()     # opcjonalnie
        honeypot = (request.form.get("website") or "").strip()  # ukryte anty-spam
        ...
        if not name or not email or not EMAIL_RE.match(email):
            ...
            return redirect(url_for("free_lesson", name=name, email=email, phone=phone))

        # Budowa obiektu danych zgodnego z istniejącą tabelą (puste nieużywane pola)
        data = {
            "name": name,
            "email": email,
            "phone": phone,
            ...
            "notes": "",
            "promo_code": "",
            "consent": True,               # akceptacja przez klik CTA pod regulaminem/PP
            "marketing_opt_in": False,     # bez checkboxa marketingowego
            "utm_source": request.args.get("utm_source"),
            "utm_medium": request.args.get("utm_medium"),
            "utm_campaign": request.args.get("utm_campaign"),
            "utm_content": request.args.get("utm_content"),
        ...
        try:
            ...
            flash("Coś poszło nie tak. Spróbuj ponownie.", "error")
            return redirect(url_for("free_lesson", name=name, email=email, phone=phone))

        # E-mail do admina (opcjonalnie)
        ADMIN_EMAIL = os.environ.get("ADMIN_EMAIL")
        if ADMIN_EMAIL:
            admin_body = (
                f"Nowy lead (trial)\\n"
                ...
                f"Telefon: {phone}\\n"
                f"IP: {data['ip']}\\n"
                f"UA: {data['user_agent'][:200]}\\n"
                f"Data: {datetime.utcnow().isoformat()} UTC\\n"
            )
            try:
                send_email(ADMIN_EMAIL, "SLAMY – nowy lead (trial)", admin_body, is_html=False)
            except Exception:
                current_app.logger.exception("Admin email error")

        # E-mail do klienta (opcjonalnie)
        try:
            client_html = f"""<!doctype html><meta charset='utf-8'>
<body style='background:#f4f6fb;padding:24px;font-family:Arial,sans-serif'>
  <div style='max-width:560px;margin:0 auto;background:#111827;color:#fff;border-radius:16px;overflow:hidden'>
    <div style='padding:14px 18px;border-bottom:1px solid rgba(255,255,255,.1)'>SLAMY – Lekcja próbna</div>
    <div style='padding:24px;font-size:15px;line-height:1.6'>
      <h1 style='margin:0 0 8px 0;font-size:20px'>Dziękujemy za zgłoszenie, {name}!</h1>
      <p>Wkrótce odezwiemy się z propozycją terminów lekcji próbnej. Jeśli masz preferowany termin, odpisz na tego maila.</p>
    </div>
    <div style='padding:16px 24px 24px 24px;font-size:12px;color:#ffffff99'>SLAMY · Learn. Speak. Connect.</div>
  </div>
</body>"""
            send_email(email, "Potwierdzenie – darmowa lekcja SLAMY", client_html, is_html=True)
        except Exception:
            current_app.logger.exception("Client email error")
