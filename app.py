...
import sqlite3
from datetime import datetime
from flask import Flask, render_template, request, redirect, url_for, flash, session, Response, current_app
from dotenv import load_dotenv
...
def free_lesson():
    if request.method == "POST":
        ...
        honeypot = (request.form.get("website") or "").strip()  # ukryte pole anty-spam

        # 1) Honeypot: jeśli bot wypełnił ukryte pole, kończymy bez komunikatu
        if honeypot:
            current_app.logger.warning("Honeypot triggered (spam bot).")
            return ("", 204)

        # 2) Minimalna walidacja
        EMAIL_RE = re.compile(r"^[^@\s]+@[^@\s]+\.[^@\s]+$")
        if not name or not email or not EMAIL_RE.match(email):
            flash("Podaj poprawne imię i e-mail.", "error")
            return redirect(url_for("free_lesson", name=name, email=email))

        # 3) Dane do zapisu (reszta pól pusta / domyślna)
        utm_source = request.args.get("utm_source")
        utm_medium = request.args.get("utm_medium")
        utm_campaign = request.args.get("utm_campaign")
        utm_content = request.args.get("utm_content")
        ...
        data = {
            ...
            "notes": "",
            "promo_code": "",
            "consent": True,              # akceptacja przez kliknięcie CTA pod linkami do regulaminu/PP
            "marketing_opt_in": False,    # brak checkboxa w formularzu
        ...
        try:
            ...
            flash("Coś poszło nie tak. Spróbuj ponownie za chwilę.", "error")
            return redirect(url_for("free_lesson", name=name, email=email))

        # 5) E-mail do admina (tekstowy, krótki)
        ADMIN_EMAIL = os.environ.get("ADMIN_EMAIL")
        admin_body = (
            f"Nowy lead (trial)\\n"
            f"Imię: {name}\\n"
        ...
        if ADMIN_EMAIL:
            try:
                send_email(ADMIN_EMAIL, "SLAMY – nowy lead (trial)", admin_body, is_html=False)
            except Exception as e:
                current_app.logger.error(f"Email admin error: {e}")

        # 6) E-mail do klienta (HTML, brand) – uproszczony
        CLIENT_FROM = os.environ.get("CLIENT_FROM_EMAIL", os.environ.get("EMAIL_USER"))
        logo_url = os.environ.get("PUBLIC_LOGO_URL", "https://slamy.online/static/logo.png")
        client_body_html = f"""
<!doctype html>
<html lang='pl'>
...
<body style='margin:0;padding:0;background:#f4f6fb;'>
  <table role='presentation' width='100%' cellspacing='0' cellpadding='0' style='background:#f4f6fb;padding:24px;font-family:Arial,sans-serif'>
    <tr><td align='center'>
      <table role='presentation' width='100%' style='max-width:560px;background:#111827;color:#fff;border-radius:16px;overflow:hidden'>
        <tr>
          <td style='padding:14px 18px;border-bottom:1px solid rgba(255,255,255,.1)'>
            <table width='100%'><tr>
              <td><img src='{logo_url}' alt='SLAMY' height='28' style='display:block;border:0;outline:0'></td>
              <td align='right' style='font-size:12px;color:#ffffff99'>Darmowa lekcja 30 min</td>
            </tr></table>
          </td>
        </tr>
        <tr>
          <td style='padding:24px 24px 12px 24px;font-size:15px;line-height:1.6'>
            <h1 style='margin:0 0 8px 0;font-size:20px'>Dziękujemy za zgłoszenie, {name}!</h1>
            <p style='margin:0 0 12px 0'>Wkrótce odezwiemy się z propozycją terminów lekcji próbnej.</p>
            <p style='margin:0'>Jeśli masz preferowany termin, odpisz na tego maila.</p>
          </td>
        </tr>
        <tr>
          <td style='padding:16px 24px 24px 24px;font-size:12px;color:#ffffff99'>
            SLAMY · Learn. Speak. Connect.
          </td>
        </tr>
      </table>
    </td></tr>
        ...
        try:
            if email:
                send_email(email, "Potwierdzenie zgłoszenia – darmowa lekcja SLAMY", client_body_html, is_html=True, from_name="SLAMY")
        except Exception as e:
            current_app.logger.error(f"Email client error: {e}")

        flash("Dziękujemy! Odezwiemy się z terminami — sprawdź skrzynkę ✉️", "success")
        return redirect(url_for("free_lesson"))

    # GET: prefill z query params
    prefill = {
